...
#' @param prefix Prefix string to prepend to `table_name` internally.
#' @rdname db
#' @importFrom dplyr tbl
#' @export
glims_tbl <- function(con, table_name, prefix = "ORAGLIMS") {
  con |> tbl(I(paste0(prefix, ".", table_name)))
}
...
#' @export
glims_join_tbl <- function(.data, table_name, by = NULL, ..., type = "left", prefix = "ORAGLIMS") {
  con <- .data[["src"]]$con
  table_y <- con |> tbl(I(paste0(prefix, ".", table_name)))
  if (type == "left") {
    .data |> left_join(table_y, ...)
  } else if (type == "right") {
    .data |> right_join(table_y, ...)
  } else if (type == "full") {
    .data |> full_join(table_y, ...)
  } else if (type == "inner") {
    .data |> inner_join(table_y, ...)
  } else {
