...
convert_glims_ordernumber(glims_order_number)

person_id_from_ordernumber(glims_order_number, convert = TRUE, db = "Oracle")
```
...
- glims_order_number:
  ...
  A GLIMS order number. The `convert_glims_ordernumber()` function will
  append this with "961" and keeps a total of 12 numbers.

- convert:

  A logical to use `convert_glims_ordernumber()` before using
  `person_id_from_ordernumber()`
...
Preferably use the `db` object (a
[list](https://rdrr.io/r/base/list.html)) to pick database columns for
filtering. It contains all GLIMS column names. For example:
...
To use precalculated values, such as using `convert_glims_ordernumber()`
to convert order numbers from the GLIMS UI, use `!!` (called
'bang-bang'):
